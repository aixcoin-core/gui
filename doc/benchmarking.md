...
Aix Core has an internal benchmarking framework, with benchmarks
...
For benchmarking, you only need to compile `bench_aix`.  The bench runner
warns if you configure with `-DCMAKE_BUILD_TYPE=Debug`, but consider if building without
it will impact the benchmark(s) you are interested in by unlatching log printers
and lock analysis.

    cmake -B build -DBUILD_BENCH=ON
    cmake --build build -t bench_aix

After compiling aix-core, the benchmarks can be run with:
...
To monitor Aix Core performance more in depth (like reindex or IBD): https://github.com/aix-dev-tools/benchcoin
