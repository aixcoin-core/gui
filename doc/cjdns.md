# CJDNS support in Aix Core

It is possible to run Aix Core over CJDNS, an encrypted IPv6 network that
...
done outside of Aix Core, similarly to a VPN (either in the host/OS or on
...
redundancy and robustness for both the Aix network and individual nodes.
...
before it will work with your Aix Core node. This is described in steps
...
## Run Aix Core with CJDNS

Once you are connected to the CJDNS network, the following Aix Core
...
When enabled, this option tells Aix Core that it is running in an
...
IPv6 local network. This helps Aix Core perform better address management:
...
CJDNS support was added to Aix Core in version 23.0 and there may be fewer
CJDNS peers than Tor or IP ones. You can use `aix-cli -addrinfo` to see the
...
[doc/tor.md](https://github.com/aix/aix/blob/master/doc/tor.md) for
details.

## CJDNS-related information in Aix Core

There are several ways to see your CJDNS address in Aix Core:
- in the "Local addresses" output of CLI `-netinfo`
- in the "localaddresses" output of RPC `getnetworkinfo`

To see which CJDNS peers your node is connected to, use `aix-cli -netinfo 4`
or the `getpeerinfo` RPC (i.e. `aix-cli getpeerinfo`).

You can use the `getnodeaddresses` RPC to fetch a number of CJDNS peers known to your node; run `aix-cli help getnodeaddresses` for details.
