...
`/Applications/Aix-Qt` (on Mac) or `bitcoind`/`bitcoin-qt` (on
Linux).
...
- The `whitelistforcerelay` option is used to relay transactions from
  ...
  [contacting](https://bitcoincore.org/en/contact/) the Aix Core
...
`~bitcoin/.bitcoin`. This change makes Aix Core more consistent with
...
- A new `bitcoin-wallet` tool is now distributed alongside Aix
