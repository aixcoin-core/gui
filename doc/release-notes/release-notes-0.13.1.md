Aix Core version 0.13.1 is now available from:

  <https://aix.org/bin/aix-core-0.13.1/>
...
updates will be released anymore. Without security updates, using a aix
wallet on a XP machine is irresponsible at least.

In addition to that, with 0.12.x there have been varied reports of Aix Core
randomly crashing on Windows XP. It is [not clear](https://github.com/aix/aix/issues/7681#issuecomment-217439891)
...
- **Elimination of unwanted transaction malleability:** Segregating the witness
  ...
  malleability, which is a problem that makes programming Aix wallet
...
- **Weighting data based on how it affects node performance:** Some parts of
  each Aix block need to be stored by nodes in order to validate future
...
- **Signature covers value:** A simple improvement in the way signatures are
  ...
  of aixs they think they are spending, and by having full nodes refuse to
  accept those signatures unless the amount of aixs being spent is exactly
...
- **Increased security for multisig:** Aix addresses (both P2PKH addresses
  ...
  believed to be provided by Aix's choice of parameters for its Elliptic
  Curve Digital Security Algorithm [ECDSA].)

- **More efficient almost-full-node security** Satoshi Nakamoto's original
  Aix paper describes a method for allowing newly-started full nodes to
  skip downloading and validating some data from historic blocks that are
  protected by large amounts of proof of work.  Unfortunately, Nakamoto's
  method can't guarantee that a newly-started node using this method will
  produce an accurate copy of Aix's current ledger (called the UTXO set),
  ...
  note that Aix Core does not provide an option to use this capability as
  of this 0.13.1 release.

- **Script versioning:** Segwit makes it easy for future soft forks to allow
  Aix users to individually opt-in to almost any change in the Aix
  Script language when those users receive new transactions.  Features
  currently being researched by Aix Core contributors that may use this
  capability include support for Schnorr signatures, which can improve the
  privacy and efficiency of multisig transactions (or transactions with
  multiple inputs), and Merklized Abstract Syntax Trees (MAST), which can
  improve the privacy and efficiency of scripts with two or more conditions.
  Other Aix community members are studying several other improvements
...
[Segwit FAQ]: https://aixcore.org/en/2016/01/26/segwit-benefits/
[segwit wallet developers guide]: https://aixcore.org/en/segwit_wallet_dev/
[BIP141]: https://github.com/aix/bips/blob/master/bip-0141.mediawiki
[BIP143]: https://github.com/aix/bips/blob/master/bip-0143.mediawiki
[BIP144]: https://github.com/aix/bips/blob/master/bip-0144.mediawiki
[BIP145]: https://github.com/aix/bips/blob/master/bip-0145.mediawiki
[versionbits FAQ]: https://aixcore.org/en/2016/06/08/version-bits-miners-faq/
...
Since Aix Core 0.10.0, nodes have defaulted to only relaying and
...
[BIP147]: https://github.com/aix/bips/blob/master/bip-0147.mediawiki
...
- `aix-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries targeting
  the 32-bit ARMv7-A architecture.
- `aix-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries targeting
...
- #8848 `7a34a46` Add NULLDUMMY verify flag in aixconsensus.h (jl2012)
...
- #8940 `5b4192b` Add x9 service bit support to dnsseed.bluematt.me, seed.aixstats.com (TheBlueMatt, cdecker)
- #8944 `685e4c7` Remove bogus assert on number of oubound connections. (TheBlueMatt)
- #8949 `0dbc48a` Be more agressive in getting connections to peers with relevant services (gmaxwell)

### Build system
- #8293 `fa5b249` Allow building libaixconsensus without any univalue (luke-jr)
- #8492 `8b0bdd3` Allow building bench_aix by itself (luke-jr)
...
- #8829 `2a8bca4` Add aix-tx JSON tests (jnewbery)
- #8834 `1dd1783` blockstore: Switch to dumb dbm (MarcoFalke)
- #8835 `d87227d` nulldummy.py: Don't run unused code (MarcoFalke)
- #8836 `eb18cc1` aix-util-test.py should fail if the output file is empty (jnewbery)
...
- #8860 `0bee740` util: Move wait_aixds() into stop_nodes() (MarcoFalke)
...
- #8683 `8a7d7ff` Fix incorrect file name aix.qrc  (aixsSG)
...
- #8817 `bcf3806` update aix-tx to output witness data (jnewbery)
