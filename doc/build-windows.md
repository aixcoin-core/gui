...
Below are some notes on how to build Aix Core for Windows.

The options known to work for building Aix Core on Windows are:
...
Note that for WSL the Aix Core source path MUST be somewhere in the default mount file system, for
example /usr/src/aix, AND not under /mnt/d/. If this is not the case the dependency autoconf scripts will fail.
...
way. This will install to `c:\workspace\aix`, for example:
```shell
cmake --install build --prefix /mnt/c/workspace/aix
```
...
```shell
cmake --install build --prefix /mnt/c/workspace/aix --strip
```
