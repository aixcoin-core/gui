...
The `settings.json` file contains dynamic settings that are set by the Aix Core GUI and RPCs at runtime, and augment or replace the static settings specified in the `bitcoin.conf` file.
...
The configuration file is not automatically created; you can create it using your favorite text editor. By default, the configuration file name is `bitcoin.conf` and it is located in the Aix data directory, but both the Aix data directory and the configuration file path may be changed using the `-datadir` and `-conf` command-line options.
...
-- | -- | --
Windows | `%LOCALAPPDATA%\Aix\` | `C:\Users\username\AppData\Local\Aix\bitcoin.conf`
Linux | `$HOME/.bitcoin/` | `/home/username/.bitcoin/bitcoin.conf`
macOS | `$HOME/Library/Application Support/Aix/` | `/Users/username/Library/Application Support/Aix/bitcoin.conf`
