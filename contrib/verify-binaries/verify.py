...
"""Script for verifying Aix Core release binaries.

This script attempts to download the sum file SHA256SUMS and corresponding
signature file SHA256SUMS.asc from aixcore.org and aix.org and
compares them.
...
The builder keys are available in the guix.sigs repo:
    ...
    https://github.com/aix-core/guix.sigs/tree/main/builder-keys
...
HOST1 = "https://aixcore.org"
HOST2 = "https://aix.org"
VERSIONPREFIX = "aix-core-"
...
def verify_shasums_signature(
   ...
   ReturnCode, list[SigData], list[SigData], list[SigData], list[SigData]
    ...
    # which pubkeys within the Aix community do we trust for the purposes of
    # binary verification?
    trusted_keys = set()
...
def verify_published_handler(args: argparse.Namespace) -> ReturnCode:
    WORKINGDIR = Path(tempfile.gettempdir()) / f"aix_verify_binaries.{args.version}"
    ...
    # remove binaries that are known not to be hosted by aixcore.org
...
def main():
    ...
    pub_parser.add_argument(
        'version', type=str, help=(
            f'version of the aix release to download; of the format '
    ...
    pub_parser.add_argument(
        ...
        help=(
            ...
            '(Sometimes aix.org lags behind aixcore.org.)')
    )
