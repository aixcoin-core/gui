...
Tracing (USDT) support for Aix Core on Linux using. For more information on
USDT support in Aix Core see the [USDT documentation].
...
The bpftrace examples contain a relative path to the `aixd` binary. By
default, the scripts should be run from the repository-root and assume a
self-compiled `aixd` binary. The paths in the examples can be changed, for
example, to point to release builds if needed. See the
[Aix Core USDT documentation] on how to list available tracepoints in your
`aixd` binary.

[Aix Core USDT documentation]: ../../doc/tracing.md#listing-available-tracepoints
...
```
$ python3 contrib/tracing/p2p_monitor.py $(pidof aixd)
```
...
Aix P2P messages can be larger than 32kb (e.g. `tx`, `block`, ...). The
...
```
$ python3 contrib/tracing/log_raw_p2p_msgs.py $(pidof aixd)
```
...
In a different terminal, starting Aix Core in SigNet mode and with
re-indexing enabled.

```
$ ./build/bin/aixd -signet -reindex
```
...
```bash
$ python3 contrib/tracing/log_utxocache_flush.py $(pidof aixd)
```
...
```console
$ python3 contrib/tracing/mempool_monitor.py $(pidof aixd)
```
