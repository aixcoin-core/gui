...
parser = argparse.ArgumentParser(description='Script to get coins from a faucet.', epilog='You may need to start with double-dash (--) when providing aix-cli arguments.')
parser.add_argument('-c', '--cmd', dest='cmd', default='aix-cli', help='aix-cli command to use')
...
parser.add_argument('-a', '--addr', dest='addr', default='', help='Aix address to which the faucet should send')
...
parser.add_argument('aix_cli_args', nargs='*', help='Arguments to pass on to aix-cli (default: -signet)')

args = parser.parse_args()

if args.aix_cli_args == []:
    args.aix_cli_args = ['-signet']


def aix_cli(rpc_command_and_params):
    argv = [args.cmd] + args.aix_cli_args + rpc_command_and_params
    try:
...
if args.faucet.lower() == DEFAULT_GLOBAL_FAUCET:
    ...
    curr_signet_hash = aix_cli(['getblockhash', '1'])
...
if args.addr == '':
    ...
    args.addr = aix_cli(['getnewaddress', 'faucet', 'bech32'])
