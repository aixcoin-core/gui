// Copyright (c) 2015-present The Aix Core developers
                  ...
                  QMessageBox::StandardButton confirm_type = QMessageBox::Yes)
{
    ...
    entry->findChild<AixAmountField*>("payAmount")->setValue(amount);
...
void CompareBalance(WalletModel& walletModel, CAmount expected_balance, QLabel* balance_label_to_check)
{
    ...
    QString balanceComparison = AixUnits::formatWithUnit(unit, expected_balance, false, AixUnits::SeparatorStyle::ALWAYS);
...
//
//     QT_QPA_PLATFORM=xcb     build/bin/test_aix-qt  # Linux
//     QT_QPA_PLATFORM=windows build/bin/test_aix-qt  # Windows
//     QT_QPA_PLATFORM=cocoa   build/bin/test_aix-qt  # macOS
void TestGUI(interfaces::Node& node, const std::shared_ptr<CWallet>& wallet)
{
    ...
    // Amount input
    AixAmountField* amountInput = receiveCoinsDialog.findChild<AixAmountField*>("reqAmount");
...
void WalletTests::walletTests()
{
    ...
    if (QApplication::platformName() == "minimal") {
        ...
        qWarning() << "Skipping WalletTests on mac build with 'minimal' platform set due to Qt bugs. To run AppTests, invoke "
                      "with 'QT_QPA_PLATFORM=cocoa test_aix-qt' on mac, or else use a linux or windows build.";
        return;
    }
#endif
    TestGUI(m_node);
