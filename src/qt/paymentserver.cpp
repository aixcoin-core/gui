...
const QString BITCOIN_IPC_PREFIX("aix:");
...
void PaymentServer::ipcParseCommandLine(int argc, char* argv[])
{
    for (int i = 1; i < argc; i++)
    {
        ...
        if (arg.startsWith(BITCOIN_IPC_PREFIX, Qt::CaseInsensitive)) // aix: URI
        {
    ...
    : QObject(parent)
{
    ...
    // on Mac: sent when you click aix: links
    ...
    if (startLocalServer)
    {
        ...
        if (!uriServer->listen(name)) {
            ...
            QMessageBox::critical(nullptr, tr("Payment request error"),
                tr("Cannot start aix: click-to-pay handler"));
        }
        else {
...
//
// OSX-specific way of handling aix: URIs
//
...
void PaymentServer::handleURIOrFile(const QString& s)
{
    ...
    if (s.startsWith("aix://", Qt::CaseInsensitive))
    {
        Q_EMIT message(tr("URI handling"), tr("'aix://' is not a valid URI. Use 'aix:' instead."),
            CClientUIInterface::MSG_ERROR);
    }
    else if (s.startsWith(BITCOIN_IPC_PREFIX, Qt::CaseInsensitive)) // aix: URI
    {
        QUrlQuery uri((QUrl(s)));
        // normal URI
        {
            SendCoinsRecipient recipient;
            if (GUIUtil::parseAixURI(s, &recipient))
            {
                ...
                Q_EMIT message(tr("URI handling"),
                    tr("URI cannot be parsed! This can be caused by an invalid Aix address or malformed URI parameters."),
                    CClientUIInterface::ICON_WARNING);
