 ...
 *
 * Ref: https://github.com/aix/aix/pull/1026
 */
static constexpr int64_t MAX_BLOCK_TIME_GAP = 90 * 60;

const std::string AixGUI::DEFAULT_UIPLATFORM =
...
AixGUI::AixGUI(interfaces::Node& node, const PlatformStyle *_platformStyle, const NetworkStyle *networkStyle, QWidget *parent) :
    QMainWindow(parent),
    m_node(node),
    ...
    m_network_style(networkStyle)
{
    ...
    if(enableWallet)
    {
        ...
        connect(walletFrame, &WalletFrame::createWalletButtonClicked, this, &AixGUI::createWallet);
    ...
    connect(labelBlocksIcon, &GUIUtil::ClickableLabel::clicked, this, &AixGUI::showModalOverlay);
    connect(progressBar, &GUIUtil::ClickableProgressBar::clicked, this, &AixGUI::showModalOverlay);
...
void AixGUI::createActions()
{
    ...
    sendCoinsAction->setStatusTip(tr("Send coins to a Aix address"));
    ...
    receiveCoinsAction->setStatusTip(tr("Request payments (generates QR codes and aix: URIs)"));
    ...
    connect(overviewAction, &QAction::triggered, this, &AixGUI::gotoOverviewPage);
    connect(sendCoinsAction, &QAction::triggered, [this]{ showNormalIfMinimized(); });
    connect(sendCoinsAction, &QAction::triggered, [this]{ gotoSendCoinsPage(); });
    connect(receiveCoinsAction, &QAction::triggered, [this]{ showNormalIfMinimized(); });
    connect(receiveCoinsAction, &QAction::triggered, this, &AixGUI::gotoReceiveCoinsPage);
    connect(historyAction, &QAction::triggered, [this]{ showNormalIfMinimized(); });
    connect(historyAction, &QAction::triggered, this, &AixGUI::gotoHistoryPage);
#endif // ENABLE_WALLET
    ...
    signMessageAction->setStatusTip(tr("Sign messages with your Aix addresses to prove you own them"));
    verifyMessageAction = new QAction(tr("&Verify message…"), this);
    verifyMessageAction->setStatusTip(tr("Verify messages to ensure they were signed with specified Aix addresses"));
    m_load_psbt_action = new QAction(tr("&Load PSBT from file…"), this);
    m_load_psbt_action->setStatusTip(tr("Load Partially Signed Aix Transaction"));
    m_load_psbt_clipboard_action = new QAction(tr("Load PSBT from &clipboard…"), this);
    m_load_psbt_clipboard_action->setStatusTip(tr("Load Partially Signed Aix Transaction from clipboard"));
    ...
    openAction->setStatusTip(tr("Open a aix: URI"));
    ...
    showHelpMessageAction->setStatusTip(tr("Show the %1 help message to get a list with possible Aix command-line options").arg(CLIENT_NAME));
    ...
    connect(quitAction, &QAction::triggered, this, &AixGUI::quitRequested);
    connect(aboutAction, &QAction::triggered, this, &AixGUI::aboutClicked);
    connect(aboutQtAction, &QAction::triggered, qApp, QApplication::aboutQt);
    connect(optionsAction, &QAction::triggered, this, &AixGUI::optionsClicked);
    connect(showHelpMessageAction, &QAction::triggered, this, &AixGUI::showHelpMessageClicked);
    connect(openRPCConsoleAction, &QAction::triggered, this, &AixGUI::showDebugWindow);
    ...
    if(walletFrame)
    {
        ...
        connect(openAction, &QAction::triggered, this, &AixGUI::openClicked);
        connect(m_open_wallet_menu, &QMenu::aboutToShow, [this] {
            ...
            for (const auto& [path, info] : m_wallet_controller->listWalletDir()) {
                ...
                connect(action, &QAction::triggered, [this, path] {
                    ...
                    connect(activity, &OpenWalletActivity::opened, this, &AixGUI::setCurrentWallet, Qt::QueuedConnection);
        ...
        connect(m_restore_wallet_action, &QAction::triggered, [this] {
            ...
            connect(activity, &RestoreWalletActivity::restored, this, &AixGUI::setCurrentWallet, Qt::QueuedConnection);
        ...
        connect(m_create_wallet_action, &QAction::triggered, this, &AixGUI::createWallet);
        ...
        connect(m_migrate_wallet_menu, &QMenu::aboutToShow, [this] {
            ...
            for (const auto& [wallet_name, info] : m_wallet_controller->listWalletDir()) {
                ...
                connect(action, &QAction::triggered, [this, wallet_name] {
                    ...
                    connect(activity, &MigrateWalletActivity::migrated, this, &AixGUI::setCurrentWallet);
        ...
        connect(m_mask_values_action, &QAction::toggled, this, &AixGUI::setPrivacy);
        connect(m_mask_values_action, &QAction::toggled, this, &AixGUI::enableHistoryAction);
    }
#endif // ENABLE_WALLET

    connect(new QShortcut(QKeySequence(Qt::CTRL | Qt::SHIFT | Qt::Key_C), this), &QShortcut::activated, this, &AixGUI::showDebugWindowActivateConsole);
    connect(new QShortcut(QKeySequence(Qt::CTRL | Qt::SHIFT | Qt::Key_D), this), &QShortcut::activated, this, &AixGUI::showDebugWindow);
}

void AixGUI::createMenuBar()
{
    appMenuBar = menuBar();
...
void AixGUI::createToolBars()
{
    if(walletFrame)
    {
        ...
        connect(m_wallet_selector, qOverload<int>(&QComboBox::currentIndexChanged), this, &AixGUI::setCurrentWalletBySelectorIndex);
...
void AixGUI::setClientModel(ClientModel *_clientModel, interfaces::BlockAndHeaderTipInfo* tip_info)
{
    ...
    if(_clientModel)
    {
        ...
        connect(_clientModel, &ClientModel::numConnectionsChanged, this, &AixGUI::setNumConnections);
        connect(_clientModel, &ClientModel::networkActiveChanged, this, &AixGUI::setNetworkActive);
        ...
        connect(_clientModel, &ClientModel::numBlocksChanged, this, &AixGUI::setNumBlocks);
        ...
        connect(_clientModel, &ClientModel::showProgress, this, &AixGUI::showProgress);
...
#ifdef ENABLE_WALLET
void AixGUI::enableHistoryAction(bool privacy)
{
...
void AixGUI::setWalletController(WalletController* wallet_controller, bool show_loading_minimized)
{
    ...
    GUIUtil::ExceptionSafeConnect(wallet_controller, &WalletController::walletAdded, this, &AixGUI::addWallet);
    connect(wallet_controller, &WalletController::walletRemoved, this, &AixGUI::removeWallet);
...
WalletController* AixGUI::getWalletController()
{
    return m_wallet_controller;
}

void AixGUI::addWallet(WalletModel* walletModel)
{
    ...
    connect(wallet_view, &WalletView::outOfSyncWarningClicked, this, &AixGUI::showModalOverlay);
    connect(wallet_view, &WalletView::transactionClicked, this, &AixGUI::gotoHistoryPage);
    connect(wallet_view, &WalletView::coinsSent, this, &AixGUI::gotoHistoryPage);
    connect(wallet_view, &WalletView::message, [this](const QString& title, const QString& message, unsigned int style) {
        this->message(title, message, style);
    });
    connect(wallet_view, &WalletView::encryptionStatusChanged, this, &AixGUI::updateWalletStatus);
    connect(wallet_view, &WalletView::incomingTransaction, this, &AixGUI::incomingTransaction);
    connect(this, &AixGUI::setPrivacy, wallet_view, &WalletView::setPrivacy);
...
void AixGUI::removeWallet(WalletModel* walletModel)
{
...
void AixGUI::setCurrentWallet(WalletModel* wallet_model)
{
    if (!walletFrame || !m_wallet_controller) return;
...
void AixGUI::setCurrentWalletBySelectorIndex(int index)
{
    WalletModel* wallet_model = m_wallet_selector->itemData(index).value<WalletModel*>();
    if (wallet_model) setCurrentWallet(wallet_model);
}

void AixGUI::removeAllWallets()
{
    if(!walletFrame)
        return;
    setWalletActionsEnabled(false);
...
void AixGUI::setWalletActionsEnabled(bool enabled)
{
...
void AixGUI::createTrayIconMenu()
{
#ifndef Q_OS_MACOS
    if (!trayIcon) return;
#endif // Q_OS_MACOS
    ...
    show_hide_action = trayIconMenu->addAction(QString(), this, &AixGUI::toggleHidden);
...
void AixGUI::optionsClicked()
{
    openOptionsDialogWithTab(OptionsDialog::TAB_MAIN);
}

void AixGUI::aboutClicked()
{
    if(!clientModel)
        return;
...
void AixGUI::showDebugWindow()
{
    GUIUtil::bringToFront(rpcConsole);
    Q_EMIT consoleShown(rpcConsole);
}

void AixGUI::showDebugWindowActivateConsole()
{
    rpcConsole->setTabFocus(RPCConsole::TabTypes::CONSOLE);
    showDebugWindow();
}

void AixGUI::showHelpMessageClicked()
{
    GUIUtil::bringToFront(helpMessageDialog);
}

#ifdef ENABLE_WALLET
void AixGUI::openClicked()
{
...
void AixGUI::gotoOverviewPage()
{
    overviewAction->setChecked(true);
    if (walletFrame) walletFrame->gotoOverviewPage();
}

void AixGUI::gotoHistoryPage()
{
    historyAction->setChecked(true);
    if (walletFrame) walletFrame->gotoHistoryPage();
}

void AixGUI::gotoReceiveCoinsPage()
{
    receiveCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoReceiveCoinsPage();
}

void AixGUI::gotoSendCoinsPage(QString addr)
{
    sendCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoSendCoinsPage(addr);
}

void AixGUI::gotoSignMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoSignMessageTab(addr);
}

void AixGUI::gotoVerifyMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoVerifyMessageTab(addr);
}
void AixGUI::gotoLoadPSBT(bool from_clipboard)
{
    if (walletFrame) walletFrame->gotoLoadPSBT(from_clipboard);
}
#endif // ENABLE_WALLET

void AixGUI::updateNetworkState()
{
    ...
    if (m_node.getNetworkActive()) {
        ...
        tooltip = tr("%n active connection(s) to Aix network.", "", count);
    } else {
        //: A substring of the tooltip.
...
void AixGUI::setNumConnections(int count)
{
    updateNetworkState();
}

void AixGUI::setNetworkActive(bool network_active)
{
    updateNetworkState();
...
void AixGUI::updateHeadersSyncProgressLabel()
{
...
void AixGUI::updateHeadersPresyncProgressLabel(int64_t height, const QDateTime& blockDate)
{
...
void AixGUI::openOptionsDialogWithTab(OptionsDialog::Tab tab)
{
    if (!clientModel || !clientModel->getOptionsModel())
        return;

    auto dlg = new OptionsDialog(this, enableWallet);
    connect(dlg, &OptionsDialog::quitOnReset, this, &AixGUI::quitRequested);
...
void AixGUI::setNumBlocks(int count, const QDateTime& blockDate, double nVerificationProgress, SyncType synctype, SynchronizationState sync_state)
{
...
void AixGUI::createWallet()
{
#ifdef ENABLE_WALLET
    auto activity = new CreateWalletActivity(getWalletController(), this);
    connect(activity, &CreateWalletActivity::created, this, &AixGUI::setCurrentWallet);
    connect(activity, &CreateWalletActivity::created, rpcConsole, &RPCConsole::setCurrentWallet);
    activity->create();
#endif // ENABLE_WALLET
}

void AixGUI::message(const QString& title, QString message, unsigned int style, bool* ret, const QString& detailed_message)
{
...
void AixGUI::changeEvent(QEvent *e)
{
    ...
    if(e->type() == QEvent::WindowStateChange)
    {
        if(clientModel && clientModel->getOptionsModel() && clientModel->getOptionsModel()->getMinimizeToTray())
        {
            ...
            {
                QTimer::singleShot(0, this, &AixGUI::hide);
                e->ignore();
            }
            else if((wsevt->oldState() & Qt::WindowMinimized) && !isMinimized())
            {
                QTimer::singleShot(0, this, &AixGUI::show);
                e->ignore();
            }
        }
    }
...
void AixGUI::closeEvent(QCloseEvent *event)
{
#ifndef Q_OS_MACOS // Ignored on Mac
    if(clientModel && clientModel->getOptionsModel())
    {
...
void AixGUI::showEvent(QShowEvent *event)
{
...
#ifdef ENABLE_WALLET
void AixGUI::incomingTransaction(const QString& date, AixUnit unit, const CAmount& amount, const QString& type, const QString& address, const QString& label, const QString& walletName)
{
    // On new transaction, make an info balloon
    QString msg = tr("Date: %1\n").arg(date) +
                  tr("Amount: %1\n").arg(AixUnits::formatWithUnit(unit, amount, true));
...
void AixGUI::dragEnterEvent(QDragEnterEvent *event)
{
    // Accept only URIs
...
void AixGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
...
bool AixGUI::eventFilter(QObject *object, QEvent *event)
{
...
#ifdef ENABLE_WALLET
bool AixGUI::handlePaymentRequest(const SendCoinsRecipient& recipient)
{
    // URI has to be valid
...
void AixGUI::setHDStatus(bool privkeyDisabled, int hdEnabled)
{
...
void AixGUI::setEncryptionStatus(int status)
{
    switch(status)
    {
...
void AixGUI::updateWalletStatus()
{
    assert(walletFrame);
...
void AixGUI::showNormalIfMinimized(bool fToggleHidden)
{
    if(!clientModel)
        return;
...
void AixGUI::toggleHidden()
{
    showNormalIfMinimized(true);
}

void AixGUI::detectShutdown()
{
...
void AixGUI::showProgress(const QString &title, int nProgress)
{
    if (nProgress == 0) {
...
void AixGUI::showModalOverlay()
{
    if (modalOverlay && (progressBar->isVisible() || modalOverlay->isLayerVisible()))
        modalOverlay->toggleVisibility();
}

static bool ThreadSafeMessageBox(AixGUI* gui, const bilingual_str& message, unsigned int style)
{
    ...
    if (message.original != message.translated) {
        detailed_message = AixGUI::tr("Original message:") + "\n" + QString::fromStdString(message.original);
    }
...
void AixGUI::subscribeToCoreSignals()
{
...
void AixGUI::unsubscribeFromCoreSignals()
{
...
bool AixGUI::isPrivacyModeActivated() const
{
    ...
    : m_platform_style{platformStyle}
{
    ...
    QList<AixUnit> units = AixUnits::availableUnits();
    int max_width = 0;
    const QFontMetrics fm(font());
    for (const AixUnit unit : units) {
        max_width = qMax(max_width, GUIUtil::TextWidth(fm, AixUnits::longName(unit)));
    }
...
{
    menu = new QMenu(this);
    for (const AixUnit u : AixUnits::availableUnits()) {
        menu->addAction(AixUnits::longName(u))->setData(QVariant::fromValue(u));
    }
...
void UnitDisplayStatusBarControl::updateDisplayUnit(AixUnit newUnits)
{
    setText(AixUnits::longName(newUnits));
}
