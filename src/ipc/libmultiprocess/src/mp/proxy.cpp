...
Connection::~Connection()
{
    ...
    // object, which can run arbitrary blocking aix code so they have to run
    ...
    // functions run, so client objects are fully disconnected before aix
    // code in the destructors are run. This way if the aix code tries to
...
void EventLoop::addAsyncCleanup(std::function<void()> fn)
{
    ...
    //
    // In aix core, running these callbacks in the right order is
