...
void IpcSocketTest(const fs::path& datadir)
{
    ...
    BOOST_CHECK_THROW(process->bind(datadir, "test_aix", invalid_bind), std::invalid_argument);
    BOOST_CHECK_THROW(process->connect(datadir, "test_aix", invalid_bind), std::invalid_argument);

    auto bind_and_listen{[&](const std::string& bind_address) {
        std::string address{bind_address};
        int serve_fd = process->bind(datadir, "test_aix", address);
    ...
    auto connect_and_test{[&](const std::string& connect_address) {
        ...
        int connect_fd{process->connect(datadir, "test_aix", address)};
    ...
    //   Address 'unix' path '"/tmp/test_common_Aix Core/ffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffff/test_aix.sock"' exceeded maximum socket path length
    std::vector<std::string> addresses{
        strprintf("unix:%s", TempPath("aix_sock0_XXXXXX")),
        strprintf("unix:%s", TempPath("aix_sock1_XXXXXX")),
    };
