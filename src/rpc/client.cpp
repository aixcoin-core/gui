...
//! Specify whether parameter should be parsed by aix-cli as a JSON value,
 ...
 *
 * JSON parameters need to be listed here to make aix-cli parse command line
 * arguments as JSON, instead of passing them as raw strings. `JSON` and
 * `JSON_OR_STRING` formats both make `aix-cli` attempt to parse the
 ...
 * `aix-cli getblockstats <hash>` without needing to quote the hash string
 * as JSON (`'"<hash>"'`).
 *
 ...
 * to make aix-cli treat them as positional parameters when `-named` is used.
 * This prevents `aix-cli` from splitting strings like "my=wallet" into a named
 ...
 * other string parameters for that method need to be listed as well so aix-cli
 ...
 *
 * For example, the command `aix-cli -named createwallet "my=wallet"`,
