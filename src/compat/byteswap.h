...
#      define aix_builtin_bswap16(x) __builtin_bswap16(x)
#    endif
#    if __has_builtin(__builtin_bswap32)
#      define aix_builtin_bswap32(x) __builtin_bswap32(x)
#    endif
#    if __has_builtin(__builtin_bswap64)
#      define aix_builtin_bswap64(x) __builtin_bswap64(x)
#    endif
#  elif defined(_MSC_VER)
#      define aix_builtin_bswap16(x) _byteswap_ushort(x)
#      define aix_builtin_bswap32(x) _byteswap_ulong(x)
#      define aix_builtin_bswap64(x) _byteswap_uint64(x)
#  endif
#endif
...
{
#ifdef aix_builtin_bswap16
    return aix_builtin_bswap16(x);
#else
    return (x >> 8) | (x << 8);
#endif
}
...
{
#ifdef aix_builtin_bswap32
    return aix_builtin_bswap32(x);
#else
...
{
#ifdef aix_builtin_bswap64
    return aix_builtin_bswap64(x);
#else
