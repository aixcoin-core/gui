...
static void SetupAixTxArgs(ArgsManager &argsman)
{
...
static int AppInitRawTx(int argc, char* argv[])
{
    ...
    if (argc < 2 || HelpRequested(gArgs) || gArgs.GetBoolArg("-version", false)) {
        ...
        std::string strUsage = CLIENT_NAME " aix-tx utility version " + FormatFullVersion() + "\n";

        if (gArgs.GetBoolArg("-version", false)) {
            strUsage += FormatParagraph(LicenseInfo());
        } else {
            strUsage += "\n"
                "The aix-tx tool is used for creating and modifying aix transactions.\n\n"
                "aix-tx can be used with \"<hex-tx> [commands]\" to update a hex-encoded aix transaction, or with \"-create [commands]\" to create a hex-encoded aix transaction.\n"
                "\n"
                "Usage: aix-tx [options] <hex-tx> [commands]\n"
                "or:    aix-tx [options] -create [commands]\n"
                "\n";
...
static int CommandLineRawTx(int argc, char* argv[])
{
    ...
    try {
        ...
        if (!fCreateBlank) {
            ...
            // param: hex-encoded aix transaction
