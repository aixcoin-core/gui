...
`test/functional/test_framework/test_framework.py:AixTestFramework.main`.
...
BASE_SCRIPTS = [
    ...
    'interface_aix_cli.py',
    'feature_bind_extra.py',
    'mempool_resurrect.py',
    'wallet_txn_doublespend.py --mineblock',
    'tool_aix_chainstate.py',
    'tool_wallet.py',
    'tool_utils.py',
...
def main():
    ...
    enable_aixd = config["components"].getboolean("ENABLE_BITCOIND")
...
def run_tests(*, test_list, build_dir, tmpdir, jobs=1, enable_coverage=False, args=None, combined_logs_len=0, failfast=False, use_term_control, results_filepath=None):
    ...
    # Warn if aixd is already running
    try:
        # pgrep exits with code zero when one or more matching processes found
        if subprocess.run(["pgrep", "-x", "aixd"], stdout=subprocess.DEVNULL).returncode == 0:
            print("%sWARNING!%s There is already a aixd process running on this system. Tests may fail unexpectedly due to resource contention!" % (BOLD[1], BOLD[0]))
    except OSError:
        # pgrep not supported
        pass
    ...
    if shutil.disk_usage(tmpdir).free < min_space:
        print(f"{BOLD[1]}WARNING!{BOLD[0]} There may be insufficient free space in {tmpdir} to run the Aix functional test suite. "
    ...
    # a hard link or a copy on any platform. See https://github.com/aix/aix/pull/27561.
...
class RPCCoverage():
    ...
    commands per `aix-cli help` (`rpc_interface.txt`).
