...
pub fn lint_includes_build_config() -> LintResult {
    let config_path = "./cmake/aix-build-config.h.in";
        ...
        git()
            .args([
                ...
                if mode {
                    ...
                    "#include <aix-build-config.h>" // Catch redundant includes with and without the IWYU pragma
                },
                "--",
            ])
    ...
    if missing {
        return Err(format!(
            r#"
One or more files use a symbol declared in the aix-build-config.h header. However, they are not
...
even though aix-build-config.h indicates that a faster feature is available and should be used.
...
#include <aix-build-config.h> // IWYU pragma: keep
    ...
    if redundant {
        return Err(r#"
None of the files use a symbol declared in the aix-build-config.h header. However, they are including
...
pub fn lint_std_filesystem() -> LintResult {
    let found = git()
        .args([
            ...
            ":(exclude)src/aix-chainstate.cpp",
        ])
        .status()
        .expect("command error")
        .success();
